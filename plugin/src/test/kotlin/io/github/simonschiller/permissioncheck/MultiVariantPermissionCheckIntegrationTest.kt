...
class MultiVariantPermissionCheckIntegrationTest {
    ...
    @ParameterizedTest
    @ArgumentsSource(TestVersions::class)
    fun `All variants are checked as part of the default check task`(gradleVersion: String, agpVersion: String) {
        androidProject.setupBaseline()

        val buildResult = androidProject.runTask("check", gradleVersion = gradleVersion, agpVersion = agpVersion)
        assertTrue(buildResult.tasks.any { it.path == ":app:checkPermissions" })

        // Individual tasks should not be executed
        assertTrue(buildResult.tasks.none { it.path == ":app:checkDebugPermissions" })
        assertTrue(buildResult.tasks.none { it.path == ":app:checkReleasePermissions" })
    }
    ...
    fun `Reports are generated when task is executed`(gradleVersion: String, agpVersion: String) {
        androidProject.setupBaseline()
        androidProject.runTask("checkPermissions", gradleVersion = gradleVersion, agpVersion = agpVersion)
    ...
    fun `Up-to-date checks work correctly when manifest changes`(gradleVersion: String, agpVersion: String) {
        ...
        assertEquals(TaskOutcome.SUCCESS, buildResult.tasks.outcomeOf("checkPermissions"))
